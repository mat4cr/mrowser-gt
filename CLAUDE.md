...
- `CursorLayout` (FrameLayout) is the input router: it intercepts key events in `dispatchKeyEvent` and dispatches D-pad/OK/BACK/MENU to the cursor or chrome, and paints the cursor dot. This is where the remote-control scheme lives — see the control tables in the Milestone A spec/plan under `docs/superpowers/`. It consumes BACK's `ACTION_DOWN` without `startTracking`, so the Activity's back-tracking never fires `onBackPressed`; the **root BACK** (no chrome, no WebView history) is routed out via the `onExitPage` callback instead → `confirmCloseTab` (a "Close this page?" dialog that blanks the WebView and returns home). BACK is **tap-versus-hold**: the tap chain above is unchanged, but a 500ms hold fires `onLongBack` instead, which reveals the chrome bar. The routing is the **pure** `BackGesture` (`decide(action, alreadyDown, longPressed, barVisible)` → arm / begin-without-arming / ignore / run-tap-chain / swallow; it takes the key action as a raw `Int` to stay Android-free); `CursorLayout` keeps the `Handler` and flags and just applies the decision. `onLongBack` returns **Boolean** — a refused hold (fullscreen) leaves the long-press flag false, so its release still runs the tap chain.
...
- `ChromeController` / `ChromeVisibility` manage the address bar, summoned with **MENU or a BACK hold** (500ms; the hold timer never arms if the bar is already visible). The bar is **only ever opened for interaction**, so there is no passive on-load reveal and no idle auto-hide: it stays up while the user types/navigates and closes on BACK/MENU/submit. During HTML5 fullscreen `MainActivity`'s `onLongBack` **refuses** the reveal (returns false — the bar would sit under the video), which leaves `backLongPressed` false so the release falls through to the normal BACK chain and exits fullscreen. `UrlNormalizer` turns bar text into a loadable URL, or null. The ★ favorite button **toggles** the current page (add/remove) and tints `accent` when the page is saved — see `MainActivity.toggleCurrentFavorite` / `updateFavoriteIcon`.
...
- `HomeView` is the launch overlay (favorites grid + URL entry); `FavoriteDialog` edits entries. Its background **cycles**: one `GradientDrawable` whose two colours are rewritten on a tick, walking forever through the **pure** `HomeBackgrounds.ALL` palettes (20s per neighbouring pair, 120s per lap) via the **pure** `blend`. `indexFor(seed)` picks the starting palette once per launch in `bind`; the `ValueAnimator` lives in `show`/`hide` — leaving it running would tick behind the WebView and through playback — and is cancelled in `onDetachedFromWindow`. Updates are throttled to 10fps (the colours crawl, so 60fps redraws of the whole overlay buy nothing). Bottom stops carry the colour and are spread far enough apart for the fade to be visible at all; top stops stay near-black so text keeps its contrast wherever the cycle is caught. Both properties are pinned by tests. `HistoryView` is the history overlay (a vertical list of visited sites; open / clear-all / long-press-to-favorite).
